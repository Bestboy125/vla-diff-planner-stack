...
int main(int argc, char *argv[])
{
    ...
    // Observation only: no change to the flight-control state machine.
    auto state_pub = nh.advertise<std_msgs::String>("fsm_state", 1);
    ros::Time state_published(0);
    while (ros::ok())
    {
        r.sleep();
        ros::spinOnce();
        fsm.process(); // We DO NOT rely on feedback as trigger, since there is no significant performance difference through our test.
        if ((ros::Time::now() - state_published).toSec() >= 0.1)
        {
            std_msgs::String msg;
            switch (fsm.get_state()) {
                case PX4CtrlFSM::MANUAL_CTRL: msg.data = "MANUAL_CTRL"; break;
                case PX4CtrlFSM::AUTO_HOVER: msg.data = "AUTO_HOVER"; break;
                case PX4CtrlFSM::CMD_CTRL: msg.data = "CMD_CTRL"; break;
                case PX4CtrlFSM::AUTO_TAKEOFF: msg.data = "AUTO_TAKEOFF"; break;
                case PX4CtrlFSM::AUTO_LAND: msg.data = "AUTO_LAND"; break;
            }
            state_pub.publish(msg);
            state_published = ros::Time::now();
        }
    }
