...
from .schemas import BridgeCommandName, BridgeCommandRequest, PlanningPreviewRequest
...
class OnboardBridgeClient:
    ...
    async def send(self, command: dict[str, Any]) -> dict[str, Any]:
        ...
        if ack.get("type") not in {"trajectory_ack", "planning_preview_ack"}:
            raise RuntimeError("onboard bridge returned an unexpected message type")
        if ack.get("mission_id") != command["mission_id"] or ack.get("sequence") != command["sequence"]:
